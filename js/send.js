window.addEventListener('load', () => {
  ...
  const getMailListSend = (token) => {
    return new Promise((res, rej) => {

      var xhr = new XMLHttpRequest();
      xhr.open('GET', 'https://www.googleapis.com/gmail/v1/users/me/messages?q=is%3Asent', false);
  ...
  getMailListSend(token).then(res => {
    console.log(res);
    ...
    res.messages.forEach(mail => {
      getMail(token, mail.id).then(mail => {
        ...
        mailCard.innerHTML = `<div class="col-3 text-truncate">${receiver}</div>
